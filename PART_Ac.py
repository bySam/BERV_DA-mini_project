...
class Lotkavolterra_Oscillator(Model):
    def __init__(self, parameter_values=None):
        ...
        self.add_parameter(Parameter(name="delta_r", expression=0.05))
...
results_ODE = model.run(algorithm = "ODE", number_of_trajectories=1)
results_SSA = model.run(algorithm = "SSA", number_of_trajectories=1)
R_ODE = results_ODE['R']
R_SSA = results_SSA['R']
time = results['time']

#plot R time evolution deterministic with noise (delta_r = 0.05 instead of 0.2)
plt.title('R time evolution ODE with noise')
plt.xlabel('Time')
plt.ylabel('Population')

plt.plot(time, R_ODE) 
plt.show()
#plot R time evolution stochastic with noise
plt.title('R time evolution SSA with noise')
plt.plot(time, R_SSA) 
plt.show()
